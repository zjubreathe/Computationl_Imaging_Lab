...
def evaluate_net(config: Dict[str, Any]):
    ...
    if os.path.exists(model_save_config['path']):
        net.load_state_dict(torch.load(model_save_config['path'], map_location=device))
        print(f"成功加载模型权重: {model_save_config['path']}")
    else:
        raise FileNotFoundError(f"模型文件未找到: {model_save_config['path']}")
    ...
    for idx, test_path in enumerate(tests_path):
        ...
        psnr_value = psnr_metric(pred, label_tensor).item()
        ssim_value = ssim_metric(pred, label_tensor).item()

        psnr_list.append(psnr_value)
        ssim_list.append(ssim_value)

        save_path = config['predict']['output_dir']
        os.makedirs(save_path, exist_ok=True)
        save_file = os.path.join(save_path, f'{idx}.jpg')
